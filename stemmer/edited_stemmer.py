...
#### zakomentirao ja (nikola) jer mi ne treba main i ne trebam argv	
...
pravila=[re.compile(r'^('+osnova+')('+nastavak+r')$') for osnova, nastavak in [e.strip().split(' ') for e in open('stemmer/rules.txt', 'r', encoding='utf-8')]]
transformacije=[e.strip().split('\t') for e in open('stemmer/transformations.txt', 'r', encoding='utf-8')]
...
def stem(text):
	...
	for token in re.findall(r'\w+', text, re.UNICODE):
		if token.lower() in stop:
			...
			continue
		output.append(korjenuj(transformiraj(token.lower())))
	return output
